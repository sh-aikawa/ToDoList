...
public interface NexusMapper {
    ...
    @Select("SELECT content, send_at, read FROM chat WHERE send_user_id = #{myId} AND receive_user_id = #{userId} OR send_user_id = #{userId} AND receive_user_id = #{myId} ORDER BY send_at ASC")
